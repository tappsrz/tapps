...
(function (exports, ByteArray, global) {
  ...
  Package.encode = function(type, body){
    ...
    var index = 8;
    var buffer = new ByteArray(PKG_HEAD_BYTES + length + index);
...
(function() {
  ...
  pinus.init = function(params, cb){
    ...
    console.log('socket open:',Package.TYPE_HANDSHAKE);
  ...
  var initWebSocket = function(url,cb) {
    ...
    var onmessage = function(event) {
      console.log('socket onmessage:',Package.decode(event.data.slice(8)));
      processPackage(Package.decode(event.data.slice(8)), cb);
